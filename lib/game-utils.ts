...
export const DIFFICULTY_BASE_SPEED: Record<Difficulty, number> = {
  easy:   200,
  normal: 150,
  hard:   100,
};
...
export function getGameSpeed(score: number, baseSpeed: number = GAME_SPEED): number {
  const level = getLevel(score);
  return Math.max(60, baseSpeed - (level - 1) * 15);
}
